...
public class ObtenerEstadoHandler : IRequestHandler<ReqObtenerEstado, ResObtenerEstado>
{
    private readonly ITransaccionesDat _transaccionesDat;
    private readonly ILogs _logs;
    private readonly string str_clase;

    public ObtenerEstadoHandler(ITransaccionesDat transaccionesDat, ILogs logs)
    {
    ...
    public async Task<ResObtenerEstado> Handle(ReqObtenerEstado request, CancellationToken cancellationToken)
    {

        const string str_operacion = "GET_ESTADOS_MONITOREO";
        var respuesta = new ResObtenerEstado();
        respuesta.LlenarResHeader( request );

        try
        {
            await _logs.SaveHeaderLogs( request, str_operacion, MethodBase.GetCurrentMethod()!.Name, str_clase );
            var result_transacction = await _transaccionesDat.obtener_estados_monitoreo( request );

            if (result_transacction.str_codigo.Equals( "000" ))
            {
                var result = (ConjuntoDatos)result_transacction.obj_cuerpo;
                var iTransacciones = Conversions.ConvertToListClassDynamic<EstadoPago>( result, 0 );
                respuesta.lst_estados = (List<EstadoPago>)iTransacciones;
